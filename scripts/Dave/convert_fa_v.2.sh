...
#Note that this workflow is specific to Amino Acid data. For nucleotides, and if exclusion of third codon positions is desired,
#See FASconCAT-G documentation for command settings, do not trim individual gene alignments prior to exclusion of third codon positions as this will disrupt
#reading frames. In this instance, modify pipeline accordingly:
#Use FASconCAT-G to exclude third codon positions of individual alignments, and then trim individual alignments prior to concatenation step. 
...
# MAFFT alignment, see documentation, or mafft --help for commands. Alignment settings are set for greatest precision, rather than speed. 
# Note: threads are set to 4, change the number of threads if more processors are available

for f in $(ls *.fa); do mafft --maxiterate 1000 --localpair --ep 10 --thread 4 $f > $f.out; done;
